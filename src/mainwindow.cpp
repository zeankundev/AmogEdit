...
void MainWindow::init(const QString &mltPath)
{
    ...
    connect(openGLMenu, &QMenu::triggered, [&](QAction *ac) {
        ...
        if (KMessageBox::questionYesNo(this, i18n("AmogEdit needs to be restarted to change this setting. Do you want to proceed?")) != KMessageBox::Yes) {
            return;
        }
        slotRestart(false);
    });
...
void MainWindow::forceIconSet(bool force)
{
    ...
    if (KMessageBox::warningContinueCancel(this, i18n("AmogEdit needs to be restarted to apply the icon theme change. Restart now?")) ==
